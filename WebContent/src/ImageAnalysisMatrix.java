...
class ImageAnalysisMatrix extends Canvas
{
	...
	public Image imgdisp;
	ArrayList pixelslist = new ArrayList();

	public ImageAnalysisMatrix(int i, int j, int k,Image img)
	{
		//  setSize(j * k, i * k);
		Rows = i;
		Cols = j;
	...
	public String stat2D(float PixelsData[][])
	{
		//	Cnt++;
		Ratio1[0] = histogram2D(PixelsData, Rows, Cols);
	...
	public String wavelet2D(int i,float PixelsData[][])
	{
		...
		for(int l = 0; l < Rows; l++)
		{
			for(int i1 = 0; i1 < Cols / 2; i1++)
			{
				...
				for(int l2 = 0; l2 < ad.length; l2++)
					d += (double)PixelsData[l][((i1 * 2 - j / 2) + l2 + Cols) % Cols] * ad[l2];
			...
			for(int k1 = Cols / 2; k1 < Cols; k1++)
			{
				...
				for(int i3 = 0; i3 < ad1.length; i3++)
					d1 += (double)PixelsData[l][(((k1 - Cols / 2) * 2 - k / 2) + i3 + Cols) % Cols] * ad1[i3];
	...
	public void simulate(int itrNo)
	{
		...
		for(int k = 0; k < Rows; k++)
		{
			for(int l = 0; l < Cols; l++)
			{
				...
				Pixels[k][l] = (float)applyCA(af[k][l], af[k][l + 1], af[k][l + 2], af[k + 1][l], af[k + 1][l + 1], af[k + 1][l + 2], af[k + 2][l], af[k + 2][l + 1], af[k + 2][l + 2], prob[k][l]);
	...
	public void paint(Graphics g)
	{
	    ...
	    for(int i = 0; i < Rows; i++)
        {
            ...
            //	System.out.print("\nMax pixel values :"+Pixels[i][j]);
	          ...
	          //      System.out.print("\nHeight::Pixels["+i+"]["+j+"]"+":\tR :"+RedVal +"\tG:"+GreenVal+"\tB:"+BlueVal);
        ...
        //	g.setColor(Color.white);
          // 	System.out.println("Itrcnt:"+itrCnt+"\tCnt:"+Cnt);
   ...
   //     System.out.print("\n\tRedWidth::"+RedWidth+"\tGreenWidth::"+GreenWidth +"\tBlueWidth:"+BlueWidth);
      //   System.out.print("\nWidth::"+widthCnt+"\tHeight::"+heightCnt);
        //fos1.write(iterationNo+"\t"+heightCnt+"\t"+widthCnt+"\n");

       //  PitSize[iterationNo] = iterationNo+","+widthCnt+","+heightCnt;
        // PitGrowthArray[iterationNo] =iterationNo+","+RedWidth+","+GreenWidth+","+BlueWidth;
		...
		// System.out.print("\nmaxRedHeight::"+maxRedHeight+"\tmaxGreenHeight::"+maxGreenHeight+"\tmaxBlueHeight:"+maxBlueHeight);

		//  System.out.print("\nmaxRedWidth::"+maxRedWidth+"\tmaxGreenWidth::"+maxGreenWidth+"\tmaxBlueWidth:"+maxBlueWidth);
	...
	public void update(Graphics g)
	{
		...
		/*   URL path = null;
        try {
    		path = new URL("file:/c:/xampp/tomcat-7.0/webapps/Sim2DEric/images/");
    	} catch (MalformedURLException e) {
    		// TODO Auto-generated catch block
    		e.printStackTrace();
    	}
        image = getImage(path, "9.JPG");
